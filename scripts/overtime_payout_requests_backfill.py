...
def load_preflight_module() -> Any:
    path = Path(__file__).with_name('overtime_payout_requests_preflight.py')
    spec = importlib.util.spec_from_file_location('overtime_payout_requests_preflight', path)
...
def build_apply_sql(entries: list[dict[str, Any]]) -> str:
    ...
    for entry in entries:
        for row in entry['sqliteRows']:
            statements.append(
                'INSERT INTO "OvertimePayoutRequest" ("employeeId", year, month, "requestedHours", note, status, "createdAt", "updatedAt") VALUES '
                f'({sql_literal(entry["employeeId"])}, {sql_literal(entry["year"])}, {sql_literal(entry["month"])}, '
                f'{sql_literal(row["requestedHours"])}, {sql_literal(row["note"])}, {sql_literal(row["status"])}, '
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Controlled backfill for overtime payout requests SQLite -> PostgreSQL.')
    ...
    print('overtime_payout_requests_backfill')
...
if __name__ == '__main__':
    try:
        ...
        print(f'overtime payout request backfill failed: {exc}', file=sys.stderr)
        raise SystemExit(1)
