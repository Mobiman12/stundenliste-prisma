...
def load_preflight_module() -> Any:
    path = Path(__file__).with_name('monthly_closings_preflight.py')
    spec = importlib.util.spec_from_file_location('monthly_closings_preflight', path)
...
def build_insert_sql(rows: list[dict[str, Any]]) -> str:
        ...
        value_rows.append(
            ...
            + ', '.join(
                [
                    ...
                    sql_literal(row['sqliteRaw']['status']),
                    sql_literal(row['sqliteRaw']['closedAt']),
                    sql_literal(row['sqliteRaw']['closedBy']),
                ]
            )
            + ')'
        )
    ...
    return (
        'BEGIN;\n'
        'INSERT INTO "MonthlyClosing" ("employeeId", "year", "month", status, "closedAt", "closedBy")\n'
        'VALUES\n  '
        + ',\n  '.join(value_rows)
        + ';\nCOMMIT;'
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Conservative backfill for SQLite monthly_closings -> PostgreSQL MonthlyClosing.')
    ...
    if duplicate_sqlite or duplicate_pg:
        raise RuntimeError(
            json.dumps(
                {
                    'duplicateSqliteKeys': duplicate_sqlite,
                    'duplicatePgKeys': duplicate_pg,
                },
                ensure_ascii=False,
            )
        )
    ...
    blocking = {
        ...
        'orphan_pg': payload['classes']['orphan_pg'],
    }
    ...
    summary = {
        ...
        'orphanSqliteUntouched': [entry['key'] for entry in payload['classes']['orphan_sqlite']],
    }

    print('monthly_closings_backfill')
