...
function normalizeStaticToken(raw: string | null | undefined): string | null {
  if (!raw) return null;
...
async function getTillhubToken(config?: TillhubConfig | null, tenantId?: string | null): Promise<string> {
  const enforceTenantConfig = Boolean(config?.enabled);

  const tenantStaticToken = normalizeStaticToken(config?.staticToken);
  if (tenantStaticToken) {
    return tenantStaticToken;
  }
  ...
  // If the tenant has Tillhub enabled in Control-Plane, do not silently fall back to global env vars.
  // A decryption mismatch (wrong CONTROL_PLANE_ENCRYPTION_KEY) would otherwise surface as a confusing
  // "TILLHUB_PASSWORD is not set" error.
  if (enforceTenantConfig) {
    const email = config?.email?.trim();
    const password = config?.password?.trim();
    if (!email || !password) {
      throw new Error(
        'Tillhub is enabled for this tenant but credentials are missing (or could not be decrypted). ' +
          'Please re-save Tillhub credentials in Control-Plane (/admin).'
      );
    }
  }

  const staticToken = normalizeStaticToken(config?.staticToken || getEnv('TILLHUB_STATIC_TOKEN', false));
  if (staticToken) {
