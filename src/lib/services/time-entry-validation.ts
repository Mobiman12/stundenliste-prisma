...
export interface TimeEntryValidationParams {
  ...
  /// Mandatory pause rule under 6h: minimum working-time threshold in minutes.
...
export function validateTimeEntry(params: TimeEntryValidationParams): TimeEntryValidationResult {
  ...
  if (!absence) {
    ...
    const mandatoryMinWorkMinutes = Math.max(params.mandatoryPauseMinWorkMinutes ?? 0, 0);
    const rawWorkMinutes = Math.round(Math.max(0, istResult.rawHours) * 60);
    if (
      code === 'RA' &&
      mandatoryPauseSetting > 0 &&
      rawWorkMinutes > 0 &&
      rawWorkMinutes <= 360 &&
      (mandatoryMinWorkMinutes <= 0 || rawWorkMinutes + 0.9 >= mandatoryMinWorkMinutes) &&
      pauseMinutes + 0.9 < mandatoryPauseSetting
    ) {
      errors.push(
        `Für diesen Einsatz ist eine verpflichtende Pause von ${mandatoryPauseSetting} Minuten hinterlegt.`
      );
    }
  }
