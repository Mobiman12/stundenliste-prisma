...
const TENANT_INFO_CACHE = new Map<string, { tenantName: string | null; tenantSlug: string | null }>();

async function fetchTenantMailContext(tenantId: string): Promise<{ tenantName: string | null; tenantSlug: string | null }> {
  const cached = TENANT_INFO_CACHE.get(tenantId);
  if (cached) {
    return cached;
  }

  const fallbackName = process.env.TENANT_NAME?.trim() || null;
  const fallbackContext = { tenantName: fallbackName, tenantSlug: null as string | null };
  const baseUrl = process.env.CONTROL_PLANE_URL?.trim();
  if (!baseUrl) {
    TENANT_INFO_CACHE.set(tenantId, fallbackContext);
  ...
  try {
    ...
    const payload = (await response.json().catch(() => null)) as
      | { tenantName?: string | null; tenantSlug?: string | null }
      | null;
    const tenantName = payload?.tenantName?.trim() || fallbackName;
    const tenantSlug = payload?.tenantSlug?.trim()?.toLowerCase() || null;
    const resolved = { tenantName, tenantSlug };
    TENANT_INFO_CACHE.set(tenantId, resolved);
    return resolved;
  } catch {
    TENANT_INFO_CACHE.set(tenantId, fallbackContext);
    return fallbackContext;
...
function buildEmployeeLoginUrl(tenantSlug: string | null): string | null {
  if (!tenantSlug) return null;
  return `https://${tenantSlug}.timesheet.timevex.com/login?mode=employee`;
}
...
export async function notifyEmployeeAboutAdminUpload(tenantId: string, options: {
  ...
  const tenantContext = await fetchTenantMailContext(tenantId);
  const fromName = tenantContext.tenantName?.trim() || undefined;
  const loginUrl = buildEmployeeLoginUrl(tenantContext.tenantSlug);
  ...
  const portalReference = loginUrl
    ? `\nDirekt zum Login: ${loginUrl}`
    : PORTAL_HINT
      ? `\nDirekt zum Login: ${PORTAL_HINT}`
      : '';
  ...
  if (definition.sensitive) {
    try {
      await sendTextMail(employee.email, subject, baseBody, { fromName });
      return {
        status: 'sent',
        sentAt: now,
        withAttachment: false,
