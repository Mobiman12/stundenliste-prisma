...
export async function getReminderSettings(tenantId: string): Promise<ReminderSettings> {
  return readReminderSettings(tenantId);
}
...
export async function updateReminderSettings(
  ...
  if (!result.unknownKeys.length) {
    await saveReminderSettings(tenantId, result.settings);
  }
  return result;
}

export async function getReminderLogs(tenantId: string, limit = 12): Promise<ReminderLogEntry[]> {
  return listReminderLogs(tenantId, limit);
}
