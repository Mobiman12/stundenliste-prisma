...
export type CreateLeaveRequestWithStatusInput = CreateLeaveRequestInput & {
  status?: LeaveRequestStatus;
...
function mapPrismaRow(row: LeaveRequest): LeaveRequestRow {
  return {
    ...
    is_unpaid: Number(row.isUnpaid ?? 0) === 1 ? 1 : 0,
    unpaid_days: Number(row.unpaidDays ?? 0),
...
export async function createLeaveRequest(input: CreateLeaveRequestInput): Promise<number> {
  ...
  const created = await prisma.leaveRequest.create({
    data: {
      ...
      isUnpaid: input.isUnpaid ? 1 : 0,
      unpaidDays: Math.max(0, Number(input.unpaidDays ?? 0)),
    },
    select: { id: true },
  });
  return created.id;
...
export async function createLeaveRequestWithStatus(
  tenantId: string,
  input: CreateLeaveRequestWithStatusInput
): Promise<number> {
  const prisma = getPrisma();
  const created = await prisma.leaveRequest.create({
    data: {
      ...
      appliedToShiftPlan: input.appliedToShiftPlan ? 1 : 0,
      isUnpaid: input.isUnpaid ? 1 : 0,
      unpaidDays: Math.max(0, Number(input.unpaidDays ?? 0)),
    },
    select: { id: true },
  });
  const existsInTenant = await prisma.leaveRequest.findFirst({
    where: { id: created.id, employee: { tenantId } },
    select: { id: true },
  });
  if (!existsInTenant) {
    throw new Error('Antrag konnte nicht tenant-sicher erstellt werden.');
  }
  return created.id;
}
...
export async function listLeaveRequestsForDateRange(
  tenantId: string,
  startIso: string,
  endIso: string,
  limit = 2000
...
export async function listLeaveRequestsForEmployeesInDateRange(
  tenantId: string,
  employeeIds: number[],
  startIso: string,
  endIso: string,
...
export async function listLeaveRequestsForEmployeeInDateRange(
  tenantId: string,
  employeeId: number,
  startIso: string,
  endIso: string,
...
export async function deleteLeaveRequestById(
  tenantId: string,
  id: number
): Promise<void> {
  const prisma = getPrisma();
...
export async function updateLeaveRequestDateRange(
  tenantId: string,
  input: {
    id: number;
    startDate: string;
...
export async function clearLeaveRequestCancellation(
  tenantId: string,
  id: number,
  options?: { keepHistory?: boolean }
): Promise<void> {
  const prisma = getPrisma();
  const keepHistory = Boolean(options?.keepHistory);
  const updated = await prisma.leaveRequest.updateMany({
    where: { id, employee: { tenantId } },
    data: {
      cancellationRequested: 0,
      cancellationRequestedAt: keepHistory ? undefined : null,
      cancellationNote: keepHistory ? undefined : null,
      updatedAt: new Date(),
    },
  });
  if (updated.count === 0) {
