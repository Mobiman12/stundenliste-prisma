...
export async function countShiftPlanDays(employeeId: number, start: string, end: string): Promise<number> {
  const prisma = getPrisma();
  return prisma.shiftPlanDay.count({
    where: {
      employeeId,
...
export async function deleteShiftPlanDaysAfter(employeeId: number, isoDateExclusive: string): Promise<void> {
  const prisma = getPrisma();
  await prisma.shiftPlanDay.deleteMany({
    where: { employeeId, dayDate: { gt: isoDateExclusive } },
  });
}
