...
const CALENDAR_BASE_URL =
  (process.env.NEXT_PUBLIC_CALENDAR_URL ?? process.env.CALENDAR_APP_URL ?? "").trim() || "";
const BOOKING_BASE_DOMAIN =
  (process.env.BOOKING_BASE_DOMAIN ?? process.env.CALENDAR_BASE_DOMAIN ?? "").trim() || "";
...
export async function GET(request: NextRequest) {
  ...
  const calendarBaseUrl =
    BOOKING_BASE_DOMAIN && tenantSlug
      ? `https://${tenantSlug}.${BOOKING_BASE_DOMAIN}`
      : CALENDAR_BASE_URL;
  if (!calendarBaseUrl) {
    return NextResponse.json(
      { error: "CALENDAR_APP_URL/NEXT_PUBLIC_CALENDAR_URL fehlt." },
      {
        status: 503,
        headers: {
          "Retry-After": "300",
  ...
  const target = new URL("/auth/staff-sso", calendarBaseUrl);
