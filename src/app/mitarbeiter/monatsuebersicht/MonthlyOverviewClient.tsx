...
function formatCarryExpiryLabel(value: string | null | undefined, year: number): string | null {
  if (!value) return null;
  const isoMatch = /^\d{4}-\d{2}-\d{2}$/.test(value) ? value : null;
  const mdMatch = /^(\d{2})-(\d{2})$/.exec(value.trim());
  const resolved = isoMatch ?? (mdMatch ? `${year}-${mdMatch[1]}-${mdMatch[2]}` : null);
...
export default function MonthlyOverviewClient({
  ...
  const selectedYear = overview.selectedYear;
  const totals = overview.totals;
  const breakdown = overview.breakdown;
  const carryExpiryLabel =
    summary.vacation.carryExpiryEnabled && summary.vacation.carryExpiryDate
      ? formatCarryExpiryLabel(summary.vacation.carryExpiryDate, selectedYear)
      : null;
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-xs text-slate-600">
            Geplanter Überstundenabbau: {formatHours(summary.worktime.plannedOvertimeReduction)}
          </p>
          <p className="text-xs text-slate-600">
            Zukünftig verfügbar: {formatHours(summary.worktime.remainingAfterPlanned)}
          </p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
        ...
        <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <dl className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2">
              ...
              <dd className="text-base font-medium text-slate-900">
                {formatDays(summary.vacation.carryRemainingDays)}
              </dd>
            </div>
            <div className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2">
              <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">
      ...
      {showWorktime ? (
        <>
          <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="text-sm text-slate-500">
              Für das Stundenkonto zählt das Stundenkonto-Delta im Monat. Urlaub, Krankheit und Feiertage sind bezahlte Abwesenheit und führen dort nicht zu Minusstunden.
            </p>
            <dl className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              ...
              <div className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2">
                <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                  Anwesenheitsdifferenz (IST - SOLL)
                </dt>
