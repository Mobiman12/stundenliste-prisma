...
export default async function MitarbeiterUmsatzPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  for (const record of records) {
    ...
    if (!isOnOrBeforeToday(parsed)) continue;
  ...
  // Keep only months that contain actual revenue; pre-created/future
  // day rows with zero revenue should not appear in revenue analytics.
  const revenueMonths = Array.from(monthMap.values()).filter((row) => row.daysWithRevenue > 0);

  const yearMap = new Map<number, YearSummary>();
  for (const summary of revenueMonths) {
  ...
  const monthlyRows = revenueMonths
