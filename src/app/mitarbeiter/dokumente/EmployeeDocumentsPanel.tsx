          ...
          <div className="space-y-3">
            <div className="space-y-3 sm:hidden">
              {documents.map((doc) => {
                ...
                return (
                  <article
                    key={`mobile-${doc.fileName}`}
                    className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm"
                  >
                    <h3 className="break-words text-sm font-semibold text-slate-900">{doc.originalName}</h3>
                    <dl className="grid grid-cols-[auto,1fr] gap-x-3 gap-y-1 text-xs">
                      <dt className="text-slate-500">Quelle</dt>
                      <dd className="text-slate-700">{doc.uploadedBy === 'admin' ? 'Admin' : 'Selbst hochgeladen'}</dd>
