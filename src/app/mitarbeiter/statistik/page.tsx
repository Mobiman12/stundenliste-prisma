...
function isOnOrBeforeToday(isoDate: string, todayIso: string): boolean {
...
export default async function MitarbeiterStatistikPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  const parsedRecords: ParsedRecord[] = records
    .map((record) => {
      if (!isOnOrBeforeToday(record.day_date, todayIso)) {
        return null;
      }
  ...
  if (filterEnd > safeLatestIso) {
    filterEnd = safeLatestIso;
  }
