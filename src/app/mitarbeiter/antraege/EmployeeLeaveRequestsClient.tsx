...
import { useMemo, useActionState, useState, useCallback, useEffect, useRef } from 'react';
...
export default function EmployeeLeaveRequestsClient({ requests, submitAction, cancelAction }: Props) {
  ...
  const today = getTodayIsoDate();
  const [requestType, setRequestType] = useState<'' | 'vacation' | 'overtime'>('');
  ...
  const handleTypeChange = useCallback(
    (value: string) => {
      ...
      const next = value === 'overtime' ? 'overtime' : value === 'vacation' ? 'vacation' : '';
      setRequestType(next);
      setRangeError(null);
  ...
  useEffect(() => {
    if (!formState.requiresUnpaidConfirmation) return;
    const unpaidDays = Number(formState.unpaidDays ?? 0);
    const confirmed = window.confirm(
      `Der Resturlaub reicht nicht aus. ${unpaidDays.toFixed(2)} Tage würden als unbezahlter Urlaub eingetragen. Jetzt als unbezahlten Urlaub speichern?`,
    );
    if (!confirmed) return;
    setAllowUnpaid(true);
    queueMicrotask(() => {
  ...
  }, [formState.requiresUnpaidConfirmation, formState.unpaidDays]);
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form ref={formRef} action={formAction} className="space-y-6">
          ...
          <input
            type="hidden"
            name="end_date"
            value={requestType === 'vacation' ? effectiveEndDate : requestType === 'overtime' ? startDate : ''}
          />
          ...
          <input type="hidden" name="allow_unpaid" value={allowUnpaid ? '1' : '0'} />

          <div className="flex flex-col gap-2 rounded-md border border-slate-200 bg-white p-3 text-sm">
            <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">
              {requestType === 'vacation' ? 'Zeitraum' : requestType === 'overtime' ? 'Datum' : 'Zeitraum / Datum'}
            </span>
            <button
              type="button"
            ...
            {!requestType ? (
              <span className="text-xs text-slate-500">Bitte zuerst die Art des Antrags auswählen.</span>
            ) : null}
        ...
        {showRangePicker ? (
          <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/40 px-3 py-4 sm:flex sm:items-center sm:justify-center sm:px-4 sm:py-8">
            <div className="mx-auto max-h-[calc(100dvh-1.5rem)] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-4 shadow-xl sm:p-6">
              ...
              <div className="sticky bottom-0 -mx-4 mt-6 flex flex-wrap justify-end gap-2 border-t border-slate-200 bg-white px-4 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-3 text-sm sm:-mx-6 sm:px-6">
                <button
                  type="button"
                  onClick={handleRangeCancel}
              ...
              <tbody className="divide-y divide-slate-200">
                {sortedRequests.map((request) => {
                  const cancellationAllowed =
                    ...
                    !request.cancelledAt &&
                    !request.cancellationRejected &&
                    compareIso(request.endDate, today) >= 0;
                  ...
                  return (
                    <tr key={request.id} className="bg-white transition hover:bg-slate-50">
                      <td className="px-4 py-3">
                        ...
                        {request.isUnpaid ? (
                          <div className="text-xs font-semibold text-rose-700">
                            Unbezahlter Urlaub: {request.unpaidDays} Tage
                          </div>
                        ) : null}
                      ...
                      <td className="px-4 py-3 text-slate-700">
                        {request.typeLabel}
                        {request.isUnpaid ? (
                          <div className="mt-1 text-xs font-semibold text-rose-700">unbezahlt</div>
                        ) : null}
                      </td>
                      <td className="px-4 py-3">
                        ...
                        {request.cancellationRejected ? (
                          <div className="mt-1 text-xs font-medium text-red-700">
                            Stornierung abgelehnt am{' '}
                            {formatDateTime(request.decidedAt ?? request.updatedAt) ??
                              formatDate((request.decidedAt ?? request.updatedAt).slice(0, 10))}
                          </div>
                        ) : null}
                        {request.cancelledAt ? (
                      ...
                      <td className="px-4 py-3">
                        ...
                        {request.cancellationRejected ? (
                          <p className="text-xs font-medium text-red-700">
                            Stornierung wurde abgelehnt. Keine erneute Anfrage möglich.
                          </p>
                        ) : null}
                        {request.status === 'approved' &&
                        !request.cancellationRequested &&
                        ...
                        compareIso(request.endDate, today) < 0 ? (
                          <p className="text-xs font-medium text-slate-500">
                            Für vergangene, genehmigte Anträge ist keine Stornierung möglich.
                          </p>
                        ) : null}
                        {request.cancelledAt ? (
