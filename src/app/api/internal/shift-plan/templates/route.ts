...
async function resolveTenantId(raw: unknown): Promise<string | null> {
  const tenantId = typeof raw === 'string' ? raw.trim() : '';
  if (tenantId) {
    return tenantId;
  }

  const tenants = await getPrisma().employee.findMany({
    distinct: ['tenantId'],
  ...
  return tenants.length === 1 ? tenants[0]?.tenantId ?? null : null;
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const templateId = await createShiftPlanTemplate(tenantId, {
      name: payload.name,
...
export async function DELETE(req: Request) {
  ...
  try {
    await deleteShiftPlanTemplate(tenantId, templateId);
