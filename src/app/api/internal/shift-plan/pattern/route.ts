...
export async function POST(req: Request) {
  ...
  // IMPORTANT: "employees" can contain either numeric Employee IDs or personnel numbers (strings).
  // Never auto-convert a string personnel number like "11" to an Employee ID (11), as that can
  // apply changes to the wrong employee (and even cross-tenant).
  const numericEmployeeIds = rawEmployeeIds
  ...
  if (personnelNumbers.length) {
    ...
    const rows = await prisma.employee.findMany({
      // Control Plane sends StaffMember.id values (cmj...) as "personnel numbers" (legacy naming).
      // Resolve both legacy personnelNumber and the dedicated controlPlaneStaffId mapping.
      where: {
        tenantId,
        OR: [{ personnelNumber: { in: personnelNumbers } }, { controlPlaneStaffId: { in: personnelNumbers } }],
      },
      select: { id: true },
    });
  ...
  if (numericEmployeeIds.length) {
    const prisma = getPrisma();
    const rows = await prisma.employee.findMany({
      where: { tenantId, id: { in: numericEmployeeIds } },
      select: { id: true },
    });
    for (const row of rows) {
      employeeIds.push(row.id);
