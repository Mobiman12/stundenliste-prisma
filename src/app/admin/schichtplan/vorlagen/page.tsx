...
async function createTemplateAction(formData: FormData): Promise<ActionResult> {
  'use server';

  const session = await getServerAuthSession();
  if (!session?.user || session.user.roleId !== 2 || !session.tenantId) {
  ...
  try {
    ...
    await createShiftPlanTemplate(session.tenantId, {
      name: payload.name,
...
async function deleteTemplateAction(formData: FormData): Promise<ActionResult> {
  'use server';
  ...
  try {
    await deleteShiftPlanTemplate(session.tenantId, templateId);
    revalidatePath(withAppBasePath('/admin/schichtplan/vorlagen'));
    revalidatePath(withAppBasePath('/admin/schichtplan'));
    return { success: true };
  } catch (error) {
...
export default async function TemplatePage() {
  ...
  if (session.user.roleId !== 2 || !session.tenantId) {
    redirect(withAppBasePath('/mitarbeiter'));
  }

  const templates = await listWeeklyShiftTemplates(session.tenantId);
