...
async function fillShiftPlanWeekAction(formData: FormData): Promise<ActionResult> {
  ...
  const template = (await listWeeklyShiftTemplates(tenantId)).find((entry) => entry.id === templateId);
  if (!template) {
...
export default async function ShiftPlanPage({
  ...
  const plan = await getWeeklyShiftPlan(
    ...
    { week: weekParam ?? null, tenantId }
  );
  const templates = await listWeeklyShiftTemplates(tenantId);
