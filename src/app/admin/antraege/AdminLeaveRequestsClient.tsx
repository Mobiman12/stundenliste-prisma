  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <button
        ...
        disabled={pending}
        onClick={(event) => {
          const confirmed = window.confirm('Diesen Antrag wirklich dauerhaft löschen?');
          if (!confirmed) {
            event.preventDefault();
          }
        }}
        className="inline-flex items-center justify-center rounded-full bg-slate-700 px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-white transition hover:bg-slate-600 disabled:cursor-not-allowed disabled:opacity-60"
      >
        Löschen
      </button>
    </div>
      ...
      <form action={formAction} className="space-y-2">
        ...
        <div className="pt-1">
          <button
            ...
            className="inline-flex items-center justify-center rounded-full bg-slate-700 px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-white transition hover:bg-slate-600"
          >
            Antrag löschen
          </button>
        </div>
          ...
          <>
            <div className="space-y-3 sm:hidden">
              {sortedRequests.map((request) => {
                const cardHighlight = request.cancellationRequested
                ...
                return (
                  <article
                    ...
                    className={`rounded-xl border p-4 shadow-sm ${cardHighlight}`}
                  >
                    ...
                    {request.cancelledAt ? (
                      <p className="mt-2 text-xs text-slate-600">Storniert am {formatDateTime(request.cancelledAt)}</p>
                    ) : null}

                    <dl className="mt-3 grid grid-cols-[auto,1fr] gap-x-3 gap-y-1 text-xs">
                      <dt className="text-slate-500">Zeitraum</dt>
                      <dd className="text-slate-700">
                        {formatDate(request.startDate)} – {formatDate(request.endDate)} ({request.totalDays} Tage)
                      </dd>
                      {request.startTime && request.endTime ? (
                        <>
                          <dt className="text-slate-500">Zeiten</dt>
                          <dd className="text-slate-700">
                            {formatTimeLabel(request.startTime)} – {formatTimeLabel(request.endTime)}
                          </dd>
                        </>
                      ) : null}
                      <dt className="text-slate-500">Typ</dt>
                      <dd className="text-slate-700">{request.typeLabel}</dd>
                    </dl>

                    <div className="mt-3 rounded-md border border-slate-200 bg-white/80 p-3 text-xs text-slate-600">
