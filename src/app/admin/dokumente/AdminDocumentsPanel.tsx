...
function DeleteButton() {
  ...
  return (
    <button
      type="submit"
      className="inline-flex w-full items-center justify-center gap-2 rounded-md border border-red-200 px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto sm:py-1"
      disabled={pending}
    >
          ...
          <div className="space-y-3">
            <div className="space-y-3 sm:hidden">
              {unseenOverview.map((entry) => {
                ...
                return (
                  <article
                    key={`overview-mobile-${entry.employeeId}-${entry.document.fileName}`}
                    className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-4"
                  >
                    <h3 className="break-words text-sm font-semibold text-slate-900">{entry.document.originalName}</h3>
                    <p className="text-xs text-slate-700">
                      <span className="font-semibold">{entry.employeeName}</span>
                      <span className="block text-slate-500">{entry.employeeUsername}</span>
                    </p>
                    <dl className="grid grid-cols-[auto,1fr] gap-x-3 gap-y-1 text-xs">
                      <dt className="text-slate-500">Typ</dt>
                      <dd className="text-slate-700">{typeLabel}</dd>
                      <dt className="text-slate-500">Quelle</dt>
                      <dd className="text-slate-700">Mitarbeiter</dd>
                      <dt className="text-slate-500">Größe</dt>
                      <dd className="text-slate-700">{formatSize(entry.document.size)}</dd>
                      <dt className="text-slate-500">Hochgeladen</dt>
                      <dd className="text-slate-700">{formatDate(entry.document.uploadedAt)}</dd>
                      <dt className="text-slate-500">E-Mail</dt>
                      <dd className={`text-xs ${mailToneClass}`}>
                        <span>{mailStatus.label}</span>
                        {mailStatus.detail ? (
                          <span className="block text-[10px] text-slate-400">{mailStatus.detail}</span>
                        ) : null}
                      </dd>
                    </dl>
                  </article>
                ...
                <div className="space-y-3">
                  <div className="space-y-3 sm:hidden">
                    {employeeDocuments.map((doc) => {
                      ...
                      return (
                        <article
                          key={`employee-mobile-${doc.fileName}`}
                          className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm"
                        >
                          <h4 className="break-words text-sm font-semibold text-slate-900">{doc.originalName}</h4>
                          <dl className="grid grid-cols-[auto,1fr] gap-x-3 gap-y-1 text-xs">
                            <dt className="text-slate-500">Typ</dt>
                            <dd className="text-slate-700">{typeLabel}</dd>
                            <dt className="text-slate-500">Größe</dt>
                            <dd className="text-slate-700">{formatSize(doc.size)}</dd>
                            <dt className="text-slate-500">Hochgeladen</dt>
                            <dd className="text-slate-700">{formatDate(doc.uploadedAt)}</dd>
                            <dt className="text-slate-500">E-Mail</dt>
                            <dd className={`text-xs ${mailToneClass}`}>
                              <span>{mailStatus.label}</span>
                              {mailStatus.detail ? (
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            <a
                              ...
                              className="inline-flex items-center justify-center rounded-md border border-slate-300 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-100"
                            >
                              Anzeigen
                            </a>
                            <a
                              ...
                              className="inline-flex items-center justify-center rounded-md border border-brand px-3 py-2 text-xs font-medium text-brand hover:bg-brand/10"
                            >
                              Download
                            </a>
                            <form action={deleteFormAction} className="grid">
                              <input type="hidden" name="employeeId" value={selectedEmployeeId ?? ''} />
