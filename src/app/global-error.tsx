...
export default function GlobalError({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/debug-allowed", { cache: "no-store" });
        if (!res.ok) return;
      ...
      <body className="min-h-screen bg-slate-50 text-slate-900">
        <div className="mx-auto flex min-h-screen max-w-3xl flex-col items-center justify-center px-6 py-16">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
            ...
            {showDetails ? (
              <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4 text-left text-xs text-slate-700">
                <div className="font-semibold">Debug-Details (Admin)</div>
                ...
                {error?.stack ? (
                  <pre className="mt-3 whitespace-pre-wrap text-[11px] text-slate-600">
                    {error.stack}
                  </pre>
                ) : null}
              </div>
